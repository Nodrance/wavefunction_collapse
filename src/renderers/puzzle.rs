...
pub fn draw_tile_opt (x: f32, y: f32, tile_size: Vec2, tileopt: &TileChoice, textures: &HashMap<&str, Texture2D>) {
    const PUZZLE_MARGIN: f32 = 0.166666;
    const PUZZLE_SCALE: f32 = PUZZLE_MARGIN*2.0 + 1.0;
    let texture = textures.get(tileopt.texture).unwrap();
    let dest_size = if tileopt.rot90 {Vec2::new(tile_size.y*PUZZLE_SCALE, tile_size.x*PUZZLE_SCALE)} else {Vec2::new(tile_size.x*PUZZLE_SCALE, tile_size.y*PUZZLE_SCALE)};
    ...
    let x = x - tile_size.x*PUZZLE_MARGIN;
    let y = y - tile_size.y*PUZZLE_MARGIN;
